...
namespace Ex04.Menus.Delegates
{
    public class Menu
    {
        ...
        public void Show()
        {
            ...
            printMenu();
            gotCorrectInput = gotMenuChoiceSuccessfully(out menuChoice);

            while (!gotCorrectInput || menuChoice != 0)
            {
                if (gotCorrectInput && menuChoice != 0)
                {
                ...
                System.Console.Clear();
                printMenu();
                Console.Write(gotCorrectInput ? string.Empty : inputErrorMsg);
                gotCorrectInput = gotMenuChoiceSuccessfully(out menuChoice);
            }
        }
